...
class UltimatumSocial extends WP_Widget {
	...
	function __construct() {
        parent::__construct(false, $name = 'Ultimatum Social');
    }
		...
		if( !empty($instance['enable_sites']) ){
			foreach($instance['enable_sites'] as $site){
				...
				if(($style=='rounded' || $style=='circle')){
					$output .= '<a href="'.$link.'" target="_blank" style="font-size:'.$instance['size'].'px;line-height:'.$instance['size'].'px" class="'.$style.'-'.$site.'"><i class="social-icon-'.$style.'-'.$site.'"></i></a>';
				} else {
					$output .= '<a href="'.$link.'" target="_blank" style="font-size:'.$instance['size'].'px;line-height:'.$instance['size'].'px" class="'.$site.'"><i class="social-icon-'.$site.'"></i></a>';
				}
